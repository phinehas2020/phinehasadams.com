...
- 2026-03-16: Replaced the sparse homepage with a clone-inspired editorial landing page adapted from the Beaucoup mirror, but rewrote it around Phinehas Adams branding, live website inventory, and the existing `/websites-for-sale` flow. Kept the main page server-rendered and CSS-led so it stayed easy to ship.
- 2026-03-16: Production build passed after the homepage swap. Existing warnings still show up outside this change: deprecated default export from `@sanity/image-url`, stale `baseline-browser-mapping` notices, and Next's workspace-root warning because there are multiple lockfiles above the repo.
- 2026-03-24: This repo had no local `.agent` workspace. Bootstrapped `.agent/*` here so planning, contracts, reports, and verification now live next to the site code instead of only in the harness repo.
- 2026-03-24: The actual site repo has no local `AGENTS.md`. If the workflow instructions come from the harness repo, mirror them into `.agent/*` here and keep runtime edits scoped away from any unrelated in-progress user changes.
- 2026-03-24: Best shell pattern for this repo is a route-aware shared chrome in `src/app/(site)/layout.tsx` via a client `SiteChrome` component. That lets `/`, `/websites-for-sale`, and the legal pages share header/footer while demo routes stay untouched.
- 2026-03-24: The old `src/app/(site)/components/` portfolio set was fully unused by active routes. Deleting it removed the long-standing lint failure in `CanvasBackground.tsx` without affecting the live site.
- 2026-03-24: `@sanity/image-url` now wants the named `createImageUrlBuilder` export. Swapping that while centralizing preview helpers in `src/app/(site)/utils/websites.ts` is the clean pattern for website card previews.
